...
class Trader:
    ...
    def time_frame_signals(self, pair, time_frames):
        ...
        for time_frame in time_frames:
            ...
            self.trigger_orders(buy, sell, has_open_time_frame_order, has_open_time_frame_trade, time_frame, pair)
    ...
    def trigger_orders(self, buy, sell, has_open_time_frame_order, has_open_time_frame_trade, time_frame, pair):
        ...
        if self.trade_data:
            ...
            price_limit_sell = 0
        if (sell and has_open_time_frame_trade and not has_open_time_frame_order) or (price_limit_sell and has_open_time_frame_trade and not has_open_time_frame_order):
            self.sell_trigger(time_frame, pair)

    def time_frame_state(self, pair, time_frame):
        #print('time_frame_state')
        return self.time_frame_trade_state(pair['pair'], time_frame['tf']), self.time_frame_order_state(pair['pair'], time_frame['tf'], 'open')

    def time_frame_trade_state(self, pair, time_frame):
        #print('time_frame_trade_state')
        self.trade_data = self.trade.get_trade(pair, time_frame)
    ...
    def time_frame_order_state(self, pair, time_frame, status):
        ...
        order_data = self.trade.get_orders(pair, time_frame, status)
