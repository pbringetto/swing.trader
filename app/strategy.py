...
class Strategy:
    ...
    def setup(self, ohlc, htf_ohlc, time_frame, pair):
        ...
        if time_frame['strategy'] == "macd_slope":
           ...
           u.show_object('strategy data', ohlc[['close', 'volume', 'volume_mean', 'volume_std', 'rsi', 'macd_slope', 'macd_sig_slope', 'macd_hist_slope']].iloc[-1])
    ...
    def rsi_strategy(self, ohlc, last_market_state, time_frame):
        bounce = ohlc['rsi'].iloc[-1] <= int(time_frame["rsi"][0] - 10)
